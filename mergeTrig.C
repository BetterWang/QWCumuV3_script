...
void mergeTrig(int sMB = 18, int sHP = 13)
{
	bool bHIN = true;
	...
	for ( int c = 0; c < 20; c++ ) {
		for ( int np = 0; np < 4; np++ ) {
			grMB_v2[np][c]  = (TGraphErrors*) fMB->Get(Form("gr_vnPtC_2_%i_%i", np, c));
			grHP1_v2[np][c] = (TGraphErrors*) fHP1->Get(Form("gr_vnPtC_2_%i_%i", np, c));
			grHP2_v2[np][c] = (TGraphErrors*) fHP2->Get(Form("gr_vnPtC_2_%i_%i", np, c));
			grHP3_v2[np][c] = (TGraphErrors*) fHP3->Get(Form("gr_vnPtC_2_%i_%i", np, c));
			grHP4_v2[np][c] = (TGraphErrors*) fHP4->Get(Form("gr_vnPtC_2_%i_%i", np, c));

			gr_V2[np][c] = new TGraphErrors(22);
			for ( int i = 0; i < 16; i++ ) {
...
//		gr_V2[3][c]->Draw("psame");
		...
		legPt->AddEntry(grMB_v2[1][c], Form("v_{2}{4} MB %i%% < Centrality < %i%%", pCent[1][c]/2, pCent[0][c+1]/2), "p");
		legPt->AddEntry(gr_V2[1][c], Form("v_{2}{4} HP %i%% < Centrality < %i%%", pCent[1][c]/2, pCent[0][c+1]/2), "p");
