...
//	int s3 = 235; // PbPb Pb-side
	...
	InitHist(hframe_pt1, "p_{T} (GeV/c)", "v_{2}{4}");
	...
	legPt1->SetBorderSize(0);
	legPt1->SetTextSize(0.05);
	legPt1->AddEntry(gr_HIN_13_002_PbPbv24pt5, "v_{2}{4} HIN-13-002", "p");
	legPt1->Draw();
	...
	legPt2->SetBorderSize(0);
	legPt2->SetTextSize(0.05);
	legPt2->AddEntry(gr_r22[0], "v_{2}^{p}{2} / v_{2}^{Pb}{2}", "p");
	legPt2->AddEntry(gr_r24[0], "v_{2}^{p}{4} / v_{2}^{Pb}{4}", "p");

	legPt2->Draw();

	if (bPbPb) {
		cT0->SaveAs(Form("v22_pT_diff_PbPb_%i_%i_%i.pdf", s1, s2, s3));
		cT1->SaveAs(Form("v24_pT_diff_PbPb_%i_%i_%i.pdf", s1, s2, s3));
		cT2->SaveAs(Form("v26_pT_diff_PbPb_%i_%i_%i.pdf", s1, s2, s3));
		cT0r->SaveAs(Form("v2_pT_diffRatio_PbPb_%i_%i_%i.pdf", s1, s2, s3));
	} else {
		cT0->SaveAs(Form("v22_pT_diff_pPb_%i_%i_%i.pdf", s1, s2, s3));
		cT1->SaveAs(Form("v24_pT_diff_pPb_%i_%i_%i.pdf", s1, s2, s3));
		cT2->SaveAs(Form("v26_pT_diff_pPb_%i_%i_%i.pdf", s1, s2, s3));
		cT0r->SaveAs(Form("v2_pT_diffRatio_pPb_%i_%i_%i.pdf", s1, s2, s3));
	}
